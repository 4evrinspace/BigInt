...
LongNumber::LongNumber(const std::string &string_number, const size_t& accuracy) {
    ...
    *this = LongNumber();
    ...
    size_t string_len = string_number.length();
    ...
    const size_t back_zeros = (_fractional_size * DIGIT_LENGTH - (string_len - dot_position - 1));
...
LongNumber::LongNumber(const char* string_number, const size_t& accuracy) {
    std::string casted = string_number;
    (*this) = LongNumber(casted, accuracy);
}


LongNumber::LongNumber(const short& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}
LongNumber::LongNumber(const unsigned short& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}


LongNumber::LongNumber(const int& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}
LongNumber::LongNumber(const unsigned int& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}


LongNumber::LongNumber(const long long& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}
LongNumber::LongNumber(const unsigned long long& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}


LongNumber::LongNumber(const double& int_number, const size_t& accuracy) : LongNumber(std::to_string(int_number), accuracy) {}
...
bool operator<(const LongNumber& left_number, const LongNumber& right_number) {
    ...
    size_t left_integer_part = left_number._digits.size() - left_number._fractional_size;
    size_t right_integer_part = right_number._digits.size() - right_number._fractional_size;
    if (left_integer_part != right_integer_part) {
        return (left_integer_part < right_integer_part) ^ comparing_negative;  
    }
    for (int i = 0; i < std::min(left_number._digits.size(), right_number._digits.size()) ; i++) {
        if (left_number._digits[i] != right_number._digits[i]) {
            return (left_number._digits[i] < right_number._digits[i]) ^ comparing_negative;
        }
    }
    return (left_number._digits.size() < right_number._digits.size()) ^ comparing_negative;
}
...
LongNumber operator-(const LongNumber& left_number, const LongNumber& right_number) {
    ...
    return _abs_sub(left_number, left_number);
}
...
LongNumber operator*(const LongNumber &left_number, const LongNumber &right_number) {
    ...
    for (int i =  right_number._digits.size() - 1; i >= 0; i--) {
        ...
        //Lazy to overload for one operation
        power_of_10 = power_of_10 * OVER_DIGIT;
    }
    ans._clean_right_zeros();
    return ans;
}
...
LongNumber operator*(const digit_t &left_number, const LongNumber &right_number) {
...
LongNumber operator/(const LongNumber &left_number, const LongNumber &right_number) {
    //We don't know about exepction, so returning strange number like -0
    ...
    while (index < integer_part) {
        ...
        temp += left_number._digits[index];
        //find closest number, mult, that: temp >= right_number * multp
        digit_t mult = LongNumber::_find_digit_multip(temp, right_number);
        ans._digits.push_back(mult);
        if (mult != 0) {
            leading_zero = false;
        } else if (leading_zero and mult == 0) {
            ans._digits.pop_back();
        }
        temp -= right_number * mult;
        index++;
    }
    ...
    while (index < left_len) {
        temp = temp * LongNumber(10);
        temp += left_number._digits[index];
        ans._digits.push_back(LongNumber::_find_digit_multip(temp, right_number));
        temp -= right_number * ans._digits.back();
        index++;
        ans._fractional_size++;
    }
    //And adding zero to temp until we reach setted precision
    while (ans._fractional_size != precision) {
        temp = temp = temp * LongNumber(10);;
        ans._digits.push_back(LongNumber::_find_digit_multip(temp, right_number));
...
std::string LongNumber::to_string(const size_t& precision) const{
    // Number of chunks after point
    const size_t full_fraction_digits = precision / DIGIT_LENGTH;
    const size_t integer_part_size = _digits.size() - _fractional_size;
    std::string ans;
    size_t index = 0;
    ...
    ans += ".";
    for (size_t i = 0; i < std::min(full_fraction_digits, _fractional_size); i++) {
